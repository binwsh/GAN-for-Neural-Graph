...
def nn_results(hdf5, experiment, code_size_1, code_size_2):
    ...
    for fold in range(10):
        ...
        X_train, y_train, X_valid, y_valid, X_test, y_test = load_adhd.load_fold(splits)
        ...
        try:
            ...
            with tf.Session() as sess:
                ...
                results.append([accuracy, precision, recall, fscore, sensivity, specificity, auc])
        finally:
            reset()

    print(np.array(results))
    best_acc_model = np.argmax(np.array(results)[:, 0])
    print(best_acc_model)
    return [experiment] + np.array(results)[best_acc_model, :].tolist()
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        results.append(nn_results(None, experiment, code_size_1, code_size_2))
