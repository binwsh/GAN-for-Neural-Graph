...
def train_fold(train_ind, test_ind, val_ind, graph_feat, features, y, y_data, params, subject_IDs):
    ...
    x_data = Reader.feature_selection(features, y, train_ind, params['num_features'])
...
def main():
    ...
    for split in splits.keys():
        splits[split] = load_adhd.load_split(DATA_ROOT, split)
        # print(splits[split][0].shape)

    all_graph, all_age, all_gender, all_label = load_adhd.concatenate_matrices(splits)
    # print(train_ind, val_ind, test_ind)

    features = load_adhd.get_features(all_graph)
    graph = load_adhd.get_graph(all_age, all_gender, 0.1)

    num_classes = 2
    num_nodes = all_graph.shape[0]
    ...
    if args.folds == 11:  # run cross validation on all folds
        ...
        train_val = cv_splits[args.folds][0]

        # train = train_val[:int(train_val.shape[0] * 0.8)]
        # val = train_val[int(train_val.shape[0] * 0.8):]
        # test = cv_splits[args.folds][1]
        train_ind, val_ind, test_ind = load_adhd.get_split_ind(splits)

        print("---------", train_ind.shape, val_ind.shape, test_ind.shape)


        scores_acc, scores_auc, scores_lin, scores_auc_lin, fold_size = train_fold(train_ind, test_ind, val_ind, graph, features, y,
                                                         y_data, params, None)
